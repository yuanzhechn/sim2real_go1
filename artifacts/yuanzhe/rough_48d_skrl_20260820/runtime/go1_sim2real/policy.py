...
class MeanPolicy(nn.Module):
    def __init__(self, observation_dim: int = 48, action_dim: int = 12, hidden_layers: Sequence[int] = (512, 256, 128)):
        super().__init__()
        layers = []
...
def export_skrl_checkpoint(checkpoint: str | Path, output: str | Path, observation_dim: int = 48, action_dim: int = 12, hidden_layers: Sequence[int] = (512, 256, 128)) -> None:
